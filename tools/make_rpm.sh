...
# Add dependences to the repository
    ...
    #ci-build/make_libtuntap.sh $RPM_SUBTYPE
...
if [ ! -z "$CI_BUILD_REF" ]; then
    ...
    ci-build/make_remote_upload.sh $RPM_SUBTYPE "$INST"

    # Install packet from remote repository
    ci-build/make_remote_install.sh $RPM_SUBTYPE "$INST"
